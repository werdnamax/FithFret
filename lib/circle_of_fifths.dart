...
class Selection implements Comparable<Selection> {
  ...
  @override
  bool operator ==(Object other) => other is Selection && other.segment == segment && other.circleIndex == circleIndex;
...
class CircleOfFifths extends StatefulWidget {
  const CircleOfFifths({super.key});
...
class CircleOfFifthState extends State<CircleOfFifths> with TickerProviderStateMixin {
  ...
  var _circle = const CircleParams(Size.zero);
  
  final _animationMap = HashMap<Selection, AnimationController>();
  ...
  Offset _transform(BuildContext context, Offset globalPosition) {
    ...
    if (renderObject != null) {
      var position = renderObject.getTransformTo(null).getTranslation().xy;
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      onEnter: (pointer) {
        ...
        _beginTracking(size, _transform(context, pointer.position));
      },
      onHover: (pointer) {
        final size = context.size ?? Size.zero;
        _updateTracking(size, _transform(context, pointer.position));
      },
      onExit: (pointer) => _updateTracking(context.size ?? Size.zero, _transform(context, pointer.position)),
      child: CustomPaint(
  ...
  @override
  void dispose() {
    for (var controller in _animationMap.values) {
      controller.dispose();
    }
    super.dispose();
  }
...
abstract class BaseCirclePainter extends CustomPainter {
  ...
  Color sliceColor(int index, {bool isHover = false, bool isSelected = false}) {
    ...
    return Color.lerp(_colors[color1Index], _colors[color2Index], fraction) ?? _colors[color1Index];
  }
}
...
class CirceOfFifthsStatePainter extends BaseCirclePainter {
  ...
  void _onPaint(Canvas canvas, CircleParams circleParams) {
    ...
    if (_selection.circleIndex == 0) {
      drawKey(canvas, circleParams.center, circleParams.innerRadius, circleParams.midRadius, _innerFontSizeScale, _relativeMinors[index], index, color, _scale);
    }
    else {
      drawKey(canvas, circleParams.center, circleParams.midRadius + circleParams.spaceWidth, circleParams.outerRadius, _outerFontSizeScale, _majorKeys[index], index, color, _scale);
    }
  }
  ...
  void drawKey(Canvas canvas, Offset center, double innerRadius, double outerRadius, double fontSizeScale, String key, int index, Color color, double scale) {
    ...
    final shadowPaint = createArcPaint(const Color(0xaa333333), strokeWidth)..maskFilter = MaskFilter.blur(BlurStyle.outer, outerRadius / 50.0);
...
class CircleOfFifthsPainter extends BaseCirclePainter {
  ...
  void _onPaint(Canvas canvas, CircleParams circleParams) {
    ...
    if (!kIsWeb) {
      canvas.clipPath(buildClipPath(circleParams));
    }

    drawCircle(canvas, circleParams.center, circleParams.midRadius + circleParams.spaceWidth, circleParams.outerRadius, _outerFontSizeScale, _majorKeys);
    drawCircle(canvas, circleParams.center, circleParams.innerRadius, circleParams.midRadius, _innerFontSizeScale, _relativeMinors);
  }
